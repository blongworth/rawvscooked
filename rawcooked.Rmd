...
normalized SD of standards. Also test against secondary standards? Separate by
wheel and system.
...
sqlres <- paste0("SELECT wheel, tp_num, runtime, sample_name, 
		    fm_corr, sig_fm_corr
                  FROM snics_results
                  WHERE sample_type = 'S'
		    AND runtime > '", date, "'")
...
data <- inner_join(res, rawsum, by = "tp_num") %>% 
	mutate(system = substring(wheel, 1, 5), 
...
OK, now some quick plotting. First, any effect of raw ratios on precision of
individual targets? Looks like nothing too obvious.
...
wheelsum <- data %>% group_by(wheel, system) %>% 
	summarise(fm_corr.m = mean(fm_corr),
